...
export default function Footer(){
    ...
    />
        ...
        <div className="border-b    flex flex-wrap lg:flex-nowrap space-x-0 m-auto pt-20 pb-10">
            <div className="text-white relative w-1/2  ">
            <Image src={logo} className="cursor-pointer object-contain" height="20" width="160" alt=""/> 
            <ul className="my-10 hidden lg:block">
                {
                   ['Home','Projects','Services','About Us','Carrers','Blog'].map((v,i)=>{
                    return <p className="mb-4 text-secondry-900 text-[16px] text-light-gray" key={i}>{v}</p>
                   })
                }
            </ul>
            <Link className="lg:block hidden absolute  underline text-sm bottom-0 " href="hello@cinnamon.agency">
             hello@cinnamon.agency
            </Link>
            </div>
            <div className="text-white relative mt-10 lg:mt-0  w-5/6">
                <h1 className="font-montBold text-xl lg:text-3xl ">All software, zero bullshit.</h1>
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-10 mt-16 mb-10 lg:mb-36">
                                ...
                                <div className='flex flex-col gap-2'>
                                    ...
                                    <p className="text-[14px] text-light-gray">slavonska avenija 6,</p>
                                    <p className="text-[14px] text-light-gray">1000, Zagreb, Croatia</p>
                ...
                <input type="text" placeholder="Your E-mail" className="bg-black border-b py-5 outline-none w-full mb-5"></input>
                <Button text={"Subscribe"} styles="w-full"/>
                    </div>
                    <div className="w-fit m-auto lg:m-0">
                    <Image className="w-3/4 xl:w-2/5 xl:ml-auto xl:mr-7" alt="image" src={footerImg} />
            <Image className="w-4/5 xl:w-3/5 xl:ml-auto" alt="image" src={reviewed} />
                    </div>
                </div>
            </div>
        </div>
        <div className="pb-5 border-b mt-8  text-white ">
